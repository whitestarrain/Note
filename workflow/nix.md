...
- 在两台机器间通过 SSH 复制 Nix store 路径及其所有依赖
- `nix-copy-closure --to user@host /nix/store/xxx` 将本地闭包推送到远程
- `nix-copy-closure --from user@host /nix/store/xxx` 从远程拉取闭包到本地
- 常用于离线部署或在无网络的服务器上安装包

### nix-daemon

- Nix 的后台守护进程，负责执行需要 root 权限的 store 操作
- 多用户模式下所有 nix 操作通过 socket 与 nix-daemon 通信
- `systemctl status nix-daemon` 查看状态
- 普通用户通过 nix-daemon 实现安全的共享 store 访问

### nix-hash

- 计算文件或目录的 Nix 哈希值
- `nix-hash --type sha256 ./src` 计算目录的 sha256 哈希
- `nix-hash --flat --type sha256 file.tar.gz` 计算单文件哈希
- `nix-hash --to-base32 <hex-hash>` 在 hex 和 base32 格式间转换
- 主要用于获取 fetchurl/fetchFromGitHub 等需要的哈希值

### nix-instantiate

- 将 Nix 表达式求值为 .drv 文件（derivation），不进行实际构建
- `nix-instantiate '<nixpkgs>' -A hello` 求值 hello 包的 derivation
- `nix-instantiate --eval -E '1+2'` 求值纯 Nix 表达式并输出结果
- `nix-instantiate --eval --json -E '{a=1;}'` 以 JSON 格式输出
...
- 下载文件并将其添加到 Nix store，输出对应的 hash 值
- `nix-prefetch-url https://example.com/file.tar.gz` 下载并输出 sha256 hash
- `nix-prefetch-url --unpack <url>` 下载压缩包并解压后计算 hash（用于 fetchFromGitHub 等）
- `nix-prefetch-url --type sha256 <url>` 指定 hash 类型
- 常用于获取 fetchurl 中的 sha256 值
...
- `types.bool` - 布尔值，true 或 false
- `types.int` - 整数，`types.ints.unsigned` 无符号整数，`types.ints.positive` 正整数
- `types.str` - 非空字符串，`types.lines` 多行字符串（自动换行合并）
- `types.path` - 路径类型
- `types.float` - 浮点数
- `types.package` - derivation 类型
- `types.attrs` - 属性集，`types.attrsOf types.str` 指定值类型的属性集
- `types.listOf types.int` - 指定元素类型的列表
- `types.nullOr types.str` - 可为 null 的字符串
- 示例：
  ```nix
  options.myPort = lib.mkOption {
...
- `types.either typeA typeB` - 值必须满足 typeA 或 typeB 之一
- `types.enum [values...]` - 值必须是枚举列表中的某一项
- `types.oneOf [types...]` - 多种类型之一（either 的泛化版本）
- 示例：
  ```nix
  ...
  options.port = lib.mkOption {
    type = lib.types.either types.port types.str;
    # 允许数字端口或命名端口（如 "http"）
  };
  ```
...
- `types.ints.between min max` - 限制整数在 [min, max] 范围内
- 求值时若超出范围会报类型检查错误
- 示例：
  ```nix
  options.opacity = lib.mkOption {
    ...
    default = 100;
    description = "Window opacity percentage";
  };
  options.fontSize = lib.mkOption {
    type = lib.types.ints.between 6 72;
    default = 14;
  };
  ```
...
- submodule 允许在 option 中定义结构化的子选项集（类似嵌套的 module）
- 用于实现复杂的配置结构，如服务配置、用户定义等

#### submodule 类型

- `types.submodule { options = { ... }; }` 定义一组子选项
- 示例：
  ```nix
  options.myService = lib.mkOption {
    type = lib.types.submodule {
      options = {
        enable = lib.mkOption { type = types.bool; default = false; };
        port = lib.mkOption { type = types.port; default = 8080; };
        host = lib.mkOption { type = types.str; default = "localhost"; };
      };
    };
    default = {};
  };
  # 使用：config.myService.port
  ```

#### nested submodule 类型

- `types.attrsOf (types.submodule {...})` 创建动态命名的子模块集合
- 每个属性名作为 key，值为 submodule 实例
  ...
  options.users = lib.mkOption {
    type = lib.types.attrsOf (lib.types.submodule {
      options = {
        ...
        shell = lib.mkOption { type = types.path; default = "/bin/bash"; };
        groups = lib.mkOption { type = types.listOf types.str; default = []; };
      };
    });
    default = {};
  };
  # 使用：config.users.alice.shell
  ```

#### 带参数的 submodule

- submodule 可以接收参数，如 `name` 获取当前属性名，`config` 获取当前子模块配置
- 使用函数形式代替属性集来定义 submodule
- 示例：
  ```nix
    ...
    type = lib.types.attrsOf (lib.types.submodule ({ name, config, ... }: {
      options = {
        ...
        dataDir = lib.mkOption {
          type = types.path;
          default = "/var/lib/${name}";  # name 即属性名，如 "nginx"
        };
      };
    }));
  };
  # services.nginx.dataDir 默认为 "/var/lib/nginx"
  ```
...
- 根据条件返回字符串或空串
- 签名：`lib.optionalString condition string`
- 示例：
  ```nix
  lib.optionalString stdenv.isLinux "-DLINUX=1"
  # Linux 上返回 "-DLINUX=1"，其他平台返回 ""
  ```
- 常用于构建参数的条件拼接

### lib.mkOrder, lib.mkBefore 与 lib.mkAfter

- 用于控制列表类型 option 值的合并顺序
- `lib.mkBefore value` 等价于 `lib.mkOrder 500 value`，排在前面
- `lib.mkAfter value` 等价于 `lib.mkOrder 1500 value`，排在后面
- 默认优先级为 1000
- 示例：
  ```nix
  {
    environment.systemPackages = lib.mkBefore [ pkgs.vim ];
    # 确保 vim 在 systemPackages 列表的前面
  }
  ```

### lib.mkOverride, lib.mkDefault and lib.mkForce

- 用于设置 option 值的优先级，处理多模块间的冲突
- `lib.mkDefault value` 等价于 `lib.mkOverride 1000 value`（默认优先级）
- `lib.mkForce value` 等价于 `lib.mkOverride 50 value`（强制覆盖）
- 数字越小优先级越高
- 示例：
  ```nix
  {
    services.openssh.enable = lib.mkDefault true;  # 可被其他模块覆盖
    networking.firewall.enable = lib.mkForce false; # 强制覆盖其他模块的设置
  }
  ```

### pkgs.callPackage

- callPackage 是 nixpkgs 中最重要的设计模式之一
- 自动将函数参数从 pkgs 属性集中注入，减少手动传参
- 示例：
  ```nix
  # mypkg.nix
  ...
  # callPackage 自动注入 lib, stdenv, fetchurl
  # 第二个参数可以手动覆盖某些参数
  pkgs.callPackage ./mypkg.nix { stdenv = pkgs.clangStdenv; }
  ```
...
- `override` 用于修改一个 derivation 的输入参数，生成新的 derivation
- 示例：
  ```nix
  pkgs.hello.override {
    stdenv = pkgs.clangStdenv;  # 使用 clang 而不是 gcc 编译
  }
  ```
- `overrideAttrs` 用于修改 mkDerivation 的属性：
  ```nix
  ...
  ```
- `override` 修改的是传给函数的参数，`overrideAttrs` 修改的是 derivation 属性

### Overlays

- Overlay 是对整个 nixpkgs 集合进行修改的函数
- 签名：`self: super: { ... }`
  - `super` 是修改前的 pkgs，`self` 是修改后的最终 pkgs
- 示例：
  ```nix
  final: prev: {
    myHello = prev.hello.overrideAttrs (old: { pname = "my-hello"; });
    # 添加自定义包
...
- Nix 二进制缓存服务器可以避免重复编译，加速部署
- 官方缓存：`cache.nixos.org`，也可以搭建私有缓存
- 常用方案：
  - [Cachix](https://cachix.org)：托管服务，使用简单
    - `cachix use mycache` 使用缓存
    - `cachix push mycache ./result` 推送构建结果
  - [attic](https://github.com/zhaofengli/attic)：自托管方案
  - nix-serve：轻量级缓存服务器
    - `nix run nixpkgs#nix-serve -- --port 5000`
- 配置使用自定义缓存（nix.conf 或 nixConfig）：
  ```
  substituters = https://cache.nixos.org https://mycache.example.com
  trusted-public-keys = cache.nixos.org-1:... mycache:...
  ```
- 生成签名密钥：`nix-store --generate-binary-cache-key mykey secret.key public.key`
...
- `nix run nixpkgs#hello` 直接运行一个包的默认程序，不安装到环境中
- 等价于构建后执行 `$out/bin/<pname>` 或 flake 中 `apps` 属性指定的程序
- 指定 flake 中的 app：`nix run .#myapp`
- 运行远程 flake：`nix run github:user/repo`
- 适合临时运行某个工具而不污染系统环境
...
- Nix registry 是 flake 引用的快捷映射表，类似于 URL 的书签
- 查看当前 registry：`nix registry list`
- 添加自定义映射：`nix registry add mypkgs github:user/mypkgs`
- 之后即可使用 `nix shell mypkgs#hello` 引用
- 固定版本：`nix registry pin nixpkgs`（将 nixpkgs 固定到当前 rev）
- registry 分三级：global（系统）、system（NixOS）、user（用户）

<https://nix.dev/manual/nix/2.17/command-ref/new-cli/nix3-registry>

## nix repl

- `nix repl` 打开交互式 Nix 表达式求值环境
- 加载 nixpkgs：`nix repl '<nixpkgs>'` 或在 repl 中 `:l <nixpkgs>`
- 加载当前 flake：`:lf .`
- 常用 repl 命令：
  - `:help` 查看所有支持的命令
  - `:l <nixpkgs>` 加载 nixpkgs
  - `:t expr` 查看表达式类型
  - `:p expr` 强制求值并打印
  - `:q` 退出
- 适合交互式探索 nixpkgs 结构和调试 nix 表达式
...
- steam-run 提供一个完整的 FHS 兼容环境，适合运行各种预编译的 Linux 二进制
- 不仅限于 Steam 游戏，任何需要标准 Linux 库路径的程序都能用
- 使用方式：
  ```bash
  # 临时运行：在 FHS 环境中执行任意命令
  nix-shell -p steam-run --run "steam-run ./my-binary"
  # 或在 nix shell 中
  nix shell nixpkgs#steam-run -c steam-run ./my-binary
  ```
- 它比 buildFHSUserEnv 更简单，预置了大量常见库（glibc, libGL, libX11 等）
- 适合临时测试，不适合生产环境打包
