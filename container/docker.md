...
Docker 利用 Linux 核心中的资源分隔机制，例如 cgroups，以及 Linux 核心名字空间（name space），来创建独立的软件容器（containers），属于操作系统层面的虚拟化技术。
...
- 镜像的 **分层** : docker镜像是分层存储的, **最上面一层为可写层,下面所有层都是只读层** ,这要做的好处是:
  ...
  - 每一个只读层都可以单独作为镜像加载,方便排查制作镜像时的问题
...
Docker 镜像采用分层存储的方式构建，每一层代表 Dockerfile 中的一条指令。多个镜像可以共享底层，从而节省存储空间并加快构建和启动速度。
...
CentOS 7 推荐使用官方脚本或 yum 仓库安装：添加 docker-ce 源后执行 `yum install docker-ce` 即可完成安装。
...
Docker 命令分为镜像命令、容器命令、网络命令等类别，下面分别说明。
...
- `docker search xxx`
  - 选镜像的建议：优先考虑官方镜像,然后是`stars`数量多的镜像
...
- 新建并启动，后台运行 `/bin/bash`
  ...
  ```bash
  docker run -td image_id /bin/bash
  ```
...
可以使用 `docker export` 将容器的文件系统导出为 tar 包，使用 `docker import` 从 tar 包创建新的镜像。与镜像的 save/load 不同，export/import 不保留镜像层历史。
...
Docker 提供了 bridge、host、none、overlay 等网络驱动，默认使用 bridge 模式为容器分配独立的网络命名空间和虚拟网桥。
...
同一用户自定义网络中的容器可以直接通过容器名进行通信，无需手动指定 IP 地址。

### 4.6.4. 配置DNS

Docker 容器默认使用宿主机的 DNS 配置，也可以通过 `--dns` 参数或 daemon.json 中的 dns 字段自定义 DNS 服务器。
...
制作 Docker 镜像主要有两种方式：通过 `docker commit` 从已有容器生成镜像，或通过编写 Dockerfile 进行自动化构建。推荐使用 Dockerfile 方式，便于版本管理和复现。

## 5.2. 指令详解

Dockerfile 中常用指令包括 FROM（基础镜像）、RUN（执行命令）、COPY/ADD（复制文件）、CMD/ENTRYPOINT（启动命令）、ENV（环境变量）、EXPOSE（声明端口）等。

## 5.3. 多阶段构建

多阶段构建（Multi-stage Build）允许在一个 Dockerfile 中使用多个 FROM 指令，将编译环境和运行环境分离，从而显著减小最终镜像体积。
...
- 从本质上，容器其实就是一种沙盒技术
  - 就好像把应用隔离在一个盒子内，使其运行
  - 因为有了盒子边界的存在，应用与应用之间不会相互干扰。并且像集装箱一样，拿来就走，随处运行。
  - 其实这就是 PaaS 的理想状态。
...
Linux Namespace 为容器提供了进程、网络、文件系统挂载点、用户等资源的隔离，使容器内的进程看不到宿主机或其他容器的资源。

### 6.1.3. CGroups(容器限制)

Control Groups（CGroups）用于限制容器可使用的 CPU、内存、磁盘 I/O 等资源上限，防止单个容器耗尽宿主机资源。

### 6.1.4. UnionFS(文件系统隔离)

联合文件系统（如 OverlayFS）将多个目录层叠合并为一个统一视图，实现镜像的分层存储和容器的可写层隔离。

## 6.2. Windows

Docker 在 Windows 上通过 Docker Desktop 运行，底层依赖 WSL 2（Windows Subsystem for Linux 2）或 Hyper-V 虚拟化来提供 Linux 内核支持。

## 6.3. Mac

Docker 在 macOS 上同样通过 Docker Desktop 运行，底层使用轻量级虚拟机（如 Apple Hypervisor Framework）来承载 Linux 内核。
