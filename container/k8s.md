...
- 通常， **应用的配置在不同的发布环境中** (例如：开发、预发布、生产环境)会有很大的差异，其中包括：
  ...
  - 每份部署特有的配置，例如域名
  - 第三方服务的证书等
- 12-Factor原则 **要求代码和配置严格分离** ，而不应该通过代码常量的形式写在代码里面。
...
- 其中可以根据管理对象分为`本地服务`(例如本地数据库)和`第三方服务`(例如Amazon S3)。
...
Kubernetes（k8s）是云原生应用编排的事实标准，涵盖 Pod、Service、Deployment、ConfigMap、Ingress 等核心资源对象，提供自动化部署、扩缩容和运维管理能力。
