...
**[负载均衡](../Middleware/load_balance.md)**
...
> 用户规模越来越大,分布地域越来越广,地域网络环境差别很大,面临问题：**如何保证用户的访问体验,不至于因访问慢而流失用户**?

![distribute_system-28](./image/distribute_system-28.png)

- CDN加速服务器：
  - 部署在**当地运营商**
  - 把**静态资源**提前缓存到各地运营商的服务中心中
  - 网络服务会通过当地的运营商进行分发
  - 如果运营商服务中心有对应的资源，就直接返回，不再转发
...
两阶段提交协议将分布式事务分为准备（Prepare）和提交（Commit）两个阶段，由协调者统一控制各参与者的提交或回滚，是最经典的分布式事务方案。

### 5.1.2. 3PC(三阶段提交)

三阶段提交在 2PC 基础上增加了 CanCommit 预询问阶段，并引入超时机制，降低了协调者单点故障导致参与者长期阻塞的风险。
...
- 常见方案：
  - UUID：全局唯一但无序，不适合做数据库主键（索引性能差）
  - 数据库自增：简单但有单点瓶颈，可用号段模式优化（如美团Leaf-segment）
  - Redis INCR：利用Redis原子递增生成ID，性能高但依赖Redis可用性
  - Snowflake（雪花算法）：Twitter开源，64位长整型，兼顾唯一性和有序性
  - Leaf：美团开源，支持号段模式和Snowflake模式

- Snowflake算法结构（64位）：
  - 1位符号位（始终为0）
  - 41位时间戳（毫秒级，可用约69年）
  - 10位工作机器ID（5位数据中心 + 5位机器ID，最多1024个节点）
  - 12位序列号（同一毫秒内递增，每毫秒可生成4096个ID）

- Snowflake的问题与优化：
  - 时钟回拨问题：机器时间回退会导致ID重复，需要检测并等待或抛异常
  - 机器ID分配：可通过ZooKeeper或配置中心分配
  - 百度UidGenerator、美团Leaf均对Snowflake做了增强
...
通过部署多个相同服务的实例组成集群，配合负载均衡和故障转移机制，是实现系统高可用的基础手段。
...
- 6、最小连接数法
  ...
  - 尽可能地提高后端服务的利用效率，将负载合理地分流到每一台服务器。
