...
物理层负责在通信信道上传输原始比特流，定义电气特性、机械特性和传输介质规格。

# 3. 链路层

## 3.1. 什么是ARP协议

ARP（Address Resolution Protocol）用于将网络层的IP地址解析为链路层的MAC地址，通过广播请求和单播应答完成地址映射。

# 4. 网络层

## 4.1. 网络分为哪几类

按IP地址分为A类（大型网络）、B类（中型网络）、C类（小型网络）、D类（组播）、E类（保留）五类。
...
- 路由转发算法
  ...
  4. 对路由表中的每一行,用其中的子网掩码和D逐位相”与”,若结果与目的网络地址匹配,把数据报传送给下一跳的路由器, 否则执行(5).
...
IP地址冲突通常由手动配置重复IP或DHCP分配异常导致。解决方法包括使用DHCP动态分配、检查并释放冲突地址、划分合理的子网等。
...
ARQ（Automatic Repeat reQuest）是一种差错控制机制，通过确认和超时重传保证数据可靠传输，主要包括停止等待ARQ和连续ARQ两种。
...
Go-Back-N是连续ARQ协议的一种实现，发送方可连续发送多个帧，当某帧出错时，接收方丢弃该帧及之后所有帧，发送方从出错帧开始重传。
...
常见的还有哈希算法（MD5、SHA系列）、椭圆曲线加密（ECC）、Diffie-Hellman密钥交换等。
...
HTTP请求由请求行（方法、URL、协议版本）、请求头（Host、Content-Type等）、请求体（POST数据）组成；响应由响应行（协议版本、状态码、原因短语）、响应头（Content-Length等）、响应体（返回数据）组成。
...
常见方案包括：Session粘滞（负载均衡器将同一用户路由到同一服务器）、Session复制（服务器间同步）、集中存储（Redis等）、基于Token的无状态认证（JWT）。

### 6.6.6. 如果禁用浏览器 cookie，如何实现用户追踪和认证

可通过URL重写（将Session ID附加到URL参数中）、隐藏表单字段、或使用Token（如JWT存储在localStorage中通过请求头传递）等方式替代。
