...
- 但是像`.LC0`之类的标签不会出现，因为它们没有使用`.global`，所以说对于链接器是无用的。
...
- 系统表指针寄存器(System table pointer registers)
  ...
  > GDTR、LDTR和IDTR在64位模式下都是80位：16个低位是寄存器的表的大小，高64位是表的起始地址。
...
ADD和SUB指令有两个操作数：源操作数和既作源又作目的的操作数。比如：
...
注意，跳转指令要求编译器定义标签。这些标签在汇编文件内必须是唯一且私有的，对文件外是不可见的，除非使用`.global`伪指令。
...
然后发出`CALL`指令，它会把当前的指令指针压入栈，然后跳转到被调函数的代码位置。
...
ARM指令集的一个重要特性就是条件执行。每条指令中有4位表示16种可能的条件，如果条件不满足，指令被忽略。上面各种类型的跳转指令只是在最单纯的B指令上应用了各种条件而已。这些条件几乎可以应用到任何指令。
...
代替使用跳转指令和标签实现这个条件语句，我们可以对前面的比较结果对每个加法指令设置条件。无论哪个条件满足都被执行，而另一个被忽略。如下面所示（假设a和b分别存储在寄存器r0和r1中）：
...
栈是一种辅助数据结构，主要用来存储函数调用历史以及局部变量。按照约定，栈的增长方向是从高地址到低地址。`sp`寄存器保存栈指针，用来追踪栈顶内容。
...
PUSH伪指令可以完成压栈的动作，还可以把任意数量的寄存器压入栈中。使用花括号`{}`列出要压栈的寄存器列表：
