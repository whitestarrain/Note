...
使用字面字符、点号(.)通配符、字符类([abc])、预定义字符类(\d, \w, \s)等匹配单个字符。

# 匹配多个字符

使用量词控制匹配次数：*(零次或多次)、+(一次或多次)、?(零次或一次)、{n,m}(指定范围)。

# 匹配开头结尾

使用锚点 ^ 匹配字符串开头，$ 匹配字符串结尾，\b 匹配单词边界。

# 匹配分组

使用圆括号()创建捕获组，支持反向引用(\1)和非捕获组(?:)。

# 贪婪和非贪婪

默认量词是贪婪的（尽可能多匹配），在量词后加?变为非贪婪（尽可能少匹配）。
