...
- Nothing: nothing是所有类型的子类，也是null的子类,它没有具体的实例对象，常见的应用：抛出异常、程序exit、无限循环等。
...
Array,List,Set,Map 都有可变和不可变的定义方式。默认都是不可变的
...
迭代器用于遍历集合中的元素，是一种惰性求值的访问方式，适合处理大型数据集。
...
-	小知识点：
  -	如果类的传参用 val或者var进行声明，就会自动给类创建指定的属性
...
子类构造器必须先调用父类构造器，可以通过主构造器的extends子句或辅助构造器的this调用实现。
