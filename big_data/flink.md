...
Flink支持三种时间语义：Event Time（事件产生时间）、Ingestion Time（进入Flink时间）、Processing Time（算子处理时间）。

## State

Flink中的状态分为Keyed State和Operator State，用于在流处理过程中维护中间计算结果。

## Checkpoint

Checkpoint是Flink实现容错的核心机制，通过定期对状态做快照来保证exactly-once语义。
