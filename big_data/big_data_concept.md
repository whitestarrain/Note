...
20世纪70年代个人计算机兴起，数据的产生和存储从大型机逐步走向小型化和普及化。

#### 1.2.1.2. 数据库的时代

关系型数据库的出现使结构化数据的管理变得高效，Oracle、MySQL等产品成为企业数据管理的主流方案。
...
Doug Cutting基于Google三篇论文的思想，开发了开源的Hadoop框架，包含HDFS和MapReduce两个核心组件。

#### 1.2.2.3. Hadoop成功

Hadoop凭借低成本、高扩展性的优势在互联网企业中迅速普及，Yahoo、Facebook等公司率先大规模采用。

### 1.2.3. 后Hadoop时代

#### 1.2.3.1. 开源产品的商业化

Cloudera、Hortonworks等公司基于Hadoop生态提供商业化发行版和企业级服务。

#### 1.2.3.2. 云计算

AWS、Azure等云厂商将大数据能力以服务形式提供，降低了企业使用大数据技术的门槛。

### 1.2.4. NoSQL的崛起

为应对海量非结构化数据的存储需求，HBase、MongoDB、Cassandra等NoSQL数据库逐渐兴起，弥补了关系型数据库在扩展性方面的不足。
...
- 数据传输
  - 将采集到的数据可靠地传递到计算和存储系统中，常见工具有Kafka、Flume等。

- 数据计算
  - 对数据进行加工转换，包括离线批处理（MapReduce、Spark）和实时流计算（Flink、Storm）。

- 数据存储
  - 将处理后的数据持久化存储，常见方案包括HDFS、HBase、数据仓库等。

- 数据查看
  - 通过报表、可视化工具（如Kibana、Superset）或即席查询引擎展示数据分析结果。

### 1.3.3. 大数据策略

大数据策略侧重于数据分析和挖掘算法的实现，包括机器学习、数据挖掘、统计分析等，目的是从海量数据中发现规律和价值。

## 1.4. 大数据技术特点

大数据技术的核心特点包括：分布式架构实现水平扩展、容错机制保障高可用性、批流一体满足不同时效性需求。

# 2. 数据采集

## 2.1. 数据的生成

数据来源于用户行为日志、业务系统数据库、IoT设备传感器、爬虫等多种渠道。

## 2.2. 数据的采集

数据采集负责将分散在各处的原始数据汇聚到统一的存储和处理平台，常见方式包括日志采集、数据库同步（CDC）、API拉取等。
...
大数据存储需要满足海量数据的可靠存储和高效访问，主要方案包括分布式文件系统（HDFS）、列式存储、对象存储等。

# 5. 流式计算

流式计算对实时产生的数据进行即时处理，适用于实时监控、实时推荐等低延迟场景，代表技术有Flink、Storm、Spark Streaming。

# 6. 离线计算

离线计算面向大批量历史数据的处理，通常以T+1的方式进行ETL和数据分析，代表技术有MapReduce、Spark、Hive。

# 7. 数据查询

数据查询引擎为用户提供交互式SQL查询能力，常见产品有Presto、Impala、ClickHouse等。

# 8. 交互数据分析

交互式分析强调低延迟的即席查询，允许分析人员灵活探索数据，通常结合OLAP引擎实现。

# 9. 数据可视化

通过图表、仪表盘等形式直观展示数据分析结果，常用工具包括Kibana、Grafana、Superset等。

# 10. 大规模机器学习

基于大数据平台进行模型训练和推理，代表框架有Spark MLlib、TensorFlow on YARN等。
