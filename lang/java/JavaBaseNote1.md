...
Integer中 intValue()返回Integer对应的int
*/
...
set:元素是无序的（准确多说是按哈希值排序）（存入和取出的顺序不一定一致）
	...
	二：
		...
		从1.8后，又多了很多方法，具体可查阅文档上面两个是1.2时出现的
...
class 多线程05tickdemo2{
	public static void main(String[] args) {
		...
		//如果不加睡眠的话，th1 start后可能进入阻塞状态，然后flag变为false，使两个线程都进入else里
		th1.start();
		try {
			Thread.sleep(10);
		} catch (Exception e) {
...
class 多线程13join函数{
	public static void main(String[] args) throws InterruptedException {
		...
		t1.join();//得到cpu执行权 ，t1结束main才能运行			//没有做中断处理，所以不会出异常，抛一下就行了
