...
- 用途：按类型自动注入依赖对象，是 Spring 提供的注解
- 查找过程：
  - 先按类型在容器中查找匹配的 bean
  - 如果找到唯一一个，直接注入
  - 如果找到多个同类型的 bean，再按变量名作为 bean id 进行匹配
  - 如果仍然无法确定，抛出异常
- 可标注位置：构造方法、setter 方法、字段、方法参数
- required 属性：默认为 true，找不到则报错；设置为 false 时允许注入 null
- 与 @Resource 区别：@Autowired 按类型注入，是 Spring 注解；@Resource 按名称注入，是 JSR-250 标准注解
- 示例：
  ```java
  @Autowired
...
- 用途：按名称自动注入依赖对象，是 JSR-250 标准注解（javax.annotation 包下）
- 查找过程：
  - 如果指定了 name 属性，按 name 在容器中查找 bean
  - 如果未指定 name，先按字段名/setter 方法对应的属性名查找
  - 如果按名称找不到，再按类型查找
  - 如果找到多个类型匹配的 bean，抛出异常
- 关键属性：name（指定 bean id）、type（指定 bean 类型）
- 与 @Autowired 区别：@Resource 优先按名称匹配，@Autowired 优先按类型匹配
- 使用场景：当需要明确指定注入哪个 bean 时使用，可移植性更好（非 Spring 专有）
- 示例：
  ```java
...
- 用途：当容器中存在多个相同类型的 bean 时，标记其中一个为首选（主要候选者）
- 使用场景：
  - 一个接口有多个实现类都注册到容器中，@Autowired 按类型注入时出现歧义
  - 此时在某个实现类上加 @Primary，使其成为默认注入的 bean
- 注意事项：
  - 如果多个同类型 bean 都标注了 @Primary，依旧会报错
  - @Primary 的优先级低于 @Qualifier 的明确指定
- 可标注位置：类上、@Bean 方法上
- 示例：
  ```java
  @Primary
  ...
  // 此时 @Autowired UserDao 会注入 MysqlUserDao
  ```

#### 2.4.6.4. @Qulifier

- 用途：按名称限定注入的 bean，解决同类型多个 bean 的歧义问题
- 使用方式：
  - 在字段/参数上与 @Autowired 配合使用，指定要注入的 bean id
  - 在方法参数上可以单独使用（如 @Bean 方法的参数）
- 与 @Primary 区别：@Qualifier 是精确指定，优先级更高；@Primary 是设置默认值
- 进阶用法：@Qualifier 加在类上可作为分组标记，配合自定义注解实现按组注入
- 示例：
  ```java
  @Autowired
  ...
  // 方法参数上单独使用
  @Bean
  public UserService userService(@Qualifier("mysqlUserDao") UserDao dao) {
...
- 可标注位置：类上、@Bean 方法上
- 注意事项：prototype 作用域的 bean 不会被容器管理销毁，需要自行处理资源释放
- 与 proxyMode 配合：当 singleton bean 依赖 prototype bean 时，需要设置 proxyMode
- 示例：
  ```java
...
- 用途：指定当前 bean 依赖的其他 bean，确保被依赖的 bean 先于当前 bean 创建
- 使用场景：
  - bean 之间没有显式的依赖注入关系，但在初始化顺序上有要求
  - 例如：某个 bean 需要在数据库连接池初始化之后才能创建
- 影响范围：影响 bean 的创建顺序和销毁顺序（销毁顺序相反）
- 可标注位置：类上、@Bean 方法上
- 对应 xml 配置：`<bean depends-on="beanA,beanB" />`
- 示例：
  ```java
  @DependsOn({"dataSourceInitializer", "cacheManager"})
  @Component
...
- 用途：将 XML 配置文件导入到基于注解的配置类中
- 使用场景：
  - 项目从 XML 配置迁移到注解配置的过渡期
  - 第三方库只提供了 XML 配置方式
  - 需要混合使用 XML 和注解配置
- 可标注位置：@Configuration 配置类上
- 属性：value/locations 指定 XML 文件路径，支持 classpath: 和 file: 前缀
- 示例：
  ```java
  @Configuration
  ...
  // 导入多个 xml
  @ImportResource({"classpath:dao.xml", "classpath:service.xml"})
  ```

#### 2.4.7.4. @Lazy

- 用途：标记 bean 为延迟加载，容器启动时不创建，首次使用时才实例化
- 使用场景：
  - bean 的创建开销大，但不一定会被用到
  - 加速容器启动时间
  - 解决某些循环依赖问题（配合代理）
- 可标注位置：类上、@Bean 方法上、@Autowired 注入点上
- 取值：true（延迟加载，默认）、false（立即加载）
- 注入点上使用：在 @Autowired 字段上标注 @Lazy，注入的是一个代理对象，真正使用时才创建目标 bean
- 示例：
  ```java
  @Lazy
  @Component
...
- 说明：通过代码手动控制事务的开启、提交和回滚，粒度更细但侵入性强
- 两种方式：
  - TransactionTemplate（推荐）：封装了事务管理的模板方法，简化编程式事务的使用
  - PlatformTransactionManager：直接使用事务管理器 API
...
- PlatformTransactionManager 示例：
  ...
  ```
- 适用场景：事务范围需要动态控制、需要在非 public 方法中使用事务、避免 AOP 代理失效问题
...
- 场景：项目中连接多个数据库（如业务库 + 日志库），需要保证跨数据源的事务一致性
- 解决方案：
  - 方案一：每个数据源配置独立的 DataSourceTransactionManager，通过 @Transactional 的 value 属性指定使用哪个事务管理器
    ```java
    @Transactional(value = "orderTransactionManager")
    public void createOrder() { }
    ```
  - 方案二：使用 JtaTransactionManager（JTA 分布式事务），通过两阶段提交保证多数据源一致性
  - 方案三：使用 Atomikos 或 Bitronix 等第三方 XA 事务管理器
  - 方案四：最终一致性方案，如事务消息 + 补偿机制（适合微服务场景）
- 注意事项：
  - Spring 默认的 DataSourceTransactionManager 不支持跨数据源事务
  - JTA 性能开销较大，非必要不使用
  - 多数据源场景下需要确保每个 @Transactional 指定了正确的 transactionManager

## 5.5. 事务消息

- 概念：将消息发送与本地事务绑定，确保消息发送和数据库操作的原子性
- 问题背景：先操作数据库再发消息，如果发消息失败但数据库已提交，则产生不一致
- 实现思路：
  - 本地消息表方案：在本地数据库增加消息表，业务操作和写消息在同一个事务中完成，后台任务定时扫描消息表发送
  - RocketMQ 事务消息：发送半消息 -> 执行本地事务 -> 根据事务结果 commit/rollback 半消息
  - Spring 中可以结合 @TransactionalEventListener 实现事务提交后发送消息
- 示例（TransactionalEventListener）：
  ```java
  ...
  ```
- 适用场景：分布式系统中保证最终一致性，订单创建后通知库存扣减等

## 5.6. 事务拦截器

- 说明：Spring 声明式事务的底层实现依赖 TransactionInterceptor（事务拦截器）
- 核心类：
  - TransactionInterceptor：实现了 MethodInterceptor 接口，是事务的 AOP 增强逻辑
  - TransactionAttributeSource：解析 @Transactional 注解获取事务属性
  - PlatformTransactionManager：实际执行事务操作
- 工作流程：
  - 1. AOP 代理拦截目标方法调用
  - 2. TransactionInterceptor.invoke() 被触发
  - 3. 从 TransactionAttributeSource 获取事务属性（传播行为、隔离级别等）
  - 4. 通过 PlatformTransactionManager 开启事务
  - 5. 执行目标方法
  - 6. 根据执行结果决定 commit 或 rollback
- 与 BeanFactoryTransactionAttributeSourceAdvisor 的关系：该 Advisor 持有 TransactionInterceptor，负责判断哪些方法需要事务增强
...
- 概念：Spring 通过 MessageSource 接口提供国际化（i18n）支持，根据 Locale 返回不同语言的消息
- 核心接口：MessageSource，主要实现类为 ResourceBundleMessageSource 和 ReloadableResourceBundleMessageSource
- 使用步骤：
  - 1. 创建资源文件：messages.properties（默认）、messages_zh_CN.properties、messages_en_US.properties
  - 2. 配置 MessageSource bean
  - 3. 通过 MessageSource.getMessage() 获取国际化消息
- 配置示例：
  ```java
  @Bean
  public MessageSource messageSource() {
      ResourceBundleMessageSource source = new ResourceBundleMessageSource();
      source.setBasename("messages"); // 资源文件基础名
...
- 使用示例：
  ...
  ```
- ReloadableResourceBundleMessageSource：支持热加载，不需要重启应用即可刷新消息内容

## 6.3. Spring 中的事件

- 概念：Spring 提供了基于观察者模式的事件机制，实现组件间的松耦合通信
- 核心组件：
  - ApplicationEvent：事件基类，自定义事件需继承该类
  - ApplicationEventPublisher：事件发布者，通常通过 ApplicationContext 发布
  - @EventListener / ApplicationListener：事件监听器
- 使用步骤：
  - 1. 定义事件类（继承 ApplicationEvent 或直接使用任意对象）
  - 2. 发布事件（注入 ApplicationEventPublisher 调用 publishEvent）
  - 3. 监听事件（@EventListener 注解方法或实现 ApplicationListener 接口）
- 示例：
  ```java
  // 发布事件
  @Autowired
  ...
  ```
- @TransactionalEventListener：在事务提交后/回滚后触发，适合事务相关的异步操作
- 异步事件：配合 @Async 注解实现异步事件处理，避免阻塞主线程

## 6.4. @Scheduled & @EnableScheduling 定时器详解

- 用途：Spring 内置的轻量级定时任务方案，通过注解声明定时执行的方法
- 开启方式：在配置类上添加 @EnableScheduling
- @Scheduled 属性：
  - fixedRate：固定频率执行（上次开始时间算起），单位毫秒
  - fixedDelay：固定延迟执行（上次结束时间算起），单位毫秒
  - cron：cron 表达式，支持复杂调度规则
  - initialDelay：首次执行延迟时间
- cron 表达式格式：秒 分 时 日 月 星期，如 `"0 0 2 * * ?"` 表示每天凌晨 2 点
- 示例：
  ```java
  @EnableScheduling
  @Configuration
  ...
  public class MyTask {
      ...
      @Scheduled(fixedDelay = 10000)  // 上次执行完毕后10秒再执行
      public void syncData() { }
  }
  ```
- 注意事项：默认单线程执行，多个定时任务会相互阻塞；可配置 TaskScheduler 线程池解决
- 生产环境建议使用 Quartz 或 XXL-Job 等分布式调度框架

## 6.5. SPEL 表达式进行计算

- 概念：Spring Expression Language（SpEL）是 Spring 提供的运行时表达式语言，支持在运行时查询和操作对象图
- 使用场景：
  - @Value 注解中引用配置和进行计算
  - Spring Security 权限表达式
  - Spring Cache 的 key 生成
  - @ConditionalOnExpression 条件判断
- 基本语法：使用 `#{expression}` 格式
- 常用功能：
  - 属性访问：`#{user.name}`
  - 方法调用：`#{user.getName()}`
  - 运算符：`#{1 + 2}`、`#{age > 18}`
  - 三元表达式：`#{age > 18 ? 'adult' : 'minor'}`
  - 引用 bean：`#{@myService.getValue()}`
...
- 开启方式：在配置类上添加 @EnableCaching
- 核心注解：
  - @Cacheable：方法结果被缓存，后续调用直接返回缓存值
    - value/cacheNames：缓存名称
    - key：缓存的 key，支持 SpEL 表达式
    - condition：缓存条件
    - unless：否定缓存条件（方法执行后判断）
  - @CachePut：每次都执行方法并更新缓存
  - @CacheEvict：清除缓存，allEntries=true 清除整个缓存
  - @Caching：组合多个缓存操作
...
- 示例：
  ...
  public User update(User user) { return userDao.save(user); }

  @CacheEvict(value = "users", key = "#id")
  public void delete(Long id) { userDao.deleteById(id); }
  ```
- 默认使用 ConcurrentMapCacheManager（基于 ConcurrentHashMap），生产环境通常集成 Redis
...
- yml 配置方式：
  ...
  ```
- 注意事项：缓存 key 的序列化方式、TTL 设置、缓存穿透/击穿/雪崩的处理
...
- 完整生命周期演示 bean：
  ...
  public class LifeCycleBean implements BeanNameAware, BeanFactoryAware,
      ...
      @Override
      public void setBeanName(String name) { System.out.println("2. BeanNameAware.setBeanName: " + name); }

      @Override
      public void setBeanFactory(BeanFactory factory) { System.out.println("3. BeanFactoryAware.setBeanFactory"); }

      @PostConstruct
      public void postConstruct() { System.out.println("4. @PostConstruct"); }

      @Override
      public void afterPropertiesSet() { System.out.println("5. InitializingBean.afterPropertiesSet"); }

      public void customInit() { System.out.println("6. custom init-method"); }

      @PreDestroy
      public void preDestroy() { System.out.println("7. @PreDestroy"); }

      @Override
      public void destroy() { System.out.println("8. DisposableBean.destroy"); }
  }
  ```
- 执行顺序总结：构造方法 -> Aware接口回调 -> BeanPostProcessor.before -> @PostConstruct -> InitializingBean -> init-method -> BeanPostProcessor.after -> 使用 -> @PreDestroy -> DisposableBean -> destroy-method
- BeanPostProcessor 对所有 bean 生效，是容器级的；InitializingBean 等是 bean 级的

#### 7.3.5.4. 生命周期源码

- 入口方法：AbstractAutowireCapableBeanFactory.doCreateBean()
- 关键源码流程：
  - createBeanInstance()：通过反射调用构造方法创建实例
  - populateBean()：属性注入（@Autowired、@Value 等）
  - initializeBean()：初始化阶段
    - invokeAwareMethods()：回调 BeanNameAware、BeanFactoryAware 等
    - applyBeanPostProcessorsBeforeInitialization()：调用所有 BeanPostProcessor 的 postProcessBeforeInitialization
    - invokeInitMethods()：先调用 InitializingBean.afterPropertiesSet()，再调用自定义 init-method
    - applyBeanPostProcessorsAfterInitialization()：调用所有 BeanPostProcessor 的 postProcessAfterInitialization
- @PostConstruct 和 @PreDestroy 的处理：通过 CommonAnnotationBeanPostProcessor 这个 BeanPostProcessor 实现
- 销毁流程入口：DisposableBeanAdapter.destroy()，依次调用 @PreDestroy -> DisposableBean.destroy() -> destroy-method

# 8. Spring AOP 实现原理

## 8.1. 切面实现

- Spring AOP 基于动态代理在运行时为目标对象生成代理，在代理中织入切面逻辑
- 切面相关核心类：
  - AnnotationAwareAspectJAutoProxyCreator：负责解析 @Aspect 注解并创建代理，本身是一个 BeanPostProcessor
  - AspectJExpressionPointcut：解析 AspectJ 切点表达式，判断方法是否匹配
  - ReflectiveMethodInvocation：方法调用链，驱动通知（Advice）的执行
- 工作流程：
  - 1. 容器启动时注册 AnnotationAwareAspectJAutoProxyCreator
  - 2. 每个 bean 初始化后（postProcessAfterInitialization），判断是否需要代理
  - 3. 收集所有匹配当前 bean 的 Advisor（包含 Pointcut + Advice）
  - 4. 如果有匹配的 Advisor，创建代理对象替换原始 bean
  - 5. 方法调用时，代理按照 Advisor 链依次执行通知逻辑
- 通知执行顺序（环绕通知模式）：Around.before -> Before -> 目标方法 -> AfterReturning/AfterThrowing -> After -> Around.after

## 8.2. 代理创建

- Spring AOP 通过 ProxyFactory 统一创建代理，内部根据条件选择 JDK 或 CGLIB
- 选择策略（DefaultAopProxyFactory.createAopProxy）：
  - 目标类实现了接口 && 未强制使用 CGLIB -> JDK 动态代理
  - 目标类未实现接口 || proxyTargetClass=true -> CGLIB 代理
  - Spring Boot 2.x 默认 proxyTargetClass=true，即默认使用 CGLIB

### 8.2.1. Cglib 代理

- 原理：通过 ASM 字节码框架在运行时生成目标类的子类，重写非 final 方法实现增强
- 核心类：CglibAopProxy，内部使用 Enhancer 创建代理
- 拦截器：DynamicAdvisedInterceptor 实现了 MethodInterceptor，负责驱动通知链执行
- 关键流程：
  - Enhancer.setSuperclass(target) 设置父类
  - Enhancer.setCallbacks() 设置方法拦截器数组
  - 生成子类实例作为代理对象
- 限制：无法代理 final 类和 final 方法
- 优势：不要求目标类实现接口，适用范围更广

### 8.2.2. JDK 代理

- 原理：基于 java.lang.reflect.Proxy，在运行时动态生成实现指定接口的代理类
- 核心类：JdkDynamicAopProxy，实现了 InvocationHandler 接口
- 关键流程：
  - Proxy.newProxyInstance(classLoader, interfaces, invocationHandler)
  - invoke() 方法中获取匹配当前方法的拦截器链
  - 如果有拦截器，创建 ReflectiveMethodInvocation 驱动链式调用
  - 如果没有拦截器，直接反射调用目标方法
- 限制：只能代理接口方法，目标类必须实现接口
- 优势：JDK 原生支持，无需额外依赖，生成类的效率高于 CGLIB
...
- 请求处理的完整流程：
  - 1. 客户端发送 HTTP 请求到 DispatcherServlet（前端控制器）
  - 2. DispatcherServlet 调用 HandlerMapping 根据 URL 查找匹配的 Handler（Controller 方法）
  - 3. HandlerMapping 返回 HandlerExecutionChain（Handler + 拦截器链）
  - 4. DispatcherServlet 根据 Handler 类型找到对应的 HandlerAdapter
  - 5. HandlerAdapter 执行 Handler（调用 Controller 方法），返回 ModelAndView
  - 6. DispatcherServlet 将 ModelAndView 传给 ViewResolver 进行视图解析
  - 7. ViewResolver 返回具体的 View 对象
  - 8. View 渲染视图（将模型数据填入模板），返回响应给客户端
- 注意：REST 接口使用 @ResponseBody 时，HandlerAdapter 直接通过 HttpMessageConverter 将返回值写入响应体，跳过视图解析步骤

## 9.2. DispatcherServlet 初始化过程

- DispatcherServlet 继承自 HttpServlet，遵循 Servlet 生命周期
- 初始化入口：HttpServletBean.init() -> FrameworkServlet.initServletBean() -> initWebApplicationContext()
- initWebApplicationContext() 核心逻辑：
  - 获取或创建 WebApplicationContext（Spring MVC 子容器）
  - 设置父容器为 Root WebApplicationContext（Spring 根容器）
  - 调用 onRefresh() -> initStrategies() 初始化九大组件
- initStrategies() 初始化的九大组件：
  ...
  - initRequestToViewNameTranslator：视图名称转换器
  - initViewResolvers：视图解析器
  - initFlashMapManager：Flash 属性管理器

## 9.3. DispatcherServlet 处理请求过程

- 入口方法：FrameworkServlet.service() -> doDispatch()
- doDispatch() 核心流程：
  - 1. checkMultipart()：检查是否为文件上传请求
  - 2. getHandler()：遍历 HandlerMapping 列表，找到匹配的 HandlerExecutionChain
  - 3. getHandlerAdapter()：根据 Handler 类型获取对应的 HandlerAdapter
  - 4. applyPreHandle()：执行拦截器的 preHandle 方法
  - 5. handle()：通过 HandlerAdapter 执行 Controller 方法，返回 ModelAndView
  - 6. applyPostHandle()：执行拦截器的 postHandle 方法
  - 7. processDispatchResult()：处理结果（视图渲染或异常处理）
  - 8. afterCompletion()：执行拦截器的 afterCompletion 方法
- 异常处理：如果 Handler 执行抛出异常，由 HandlerExceptionResolver 处理

## 9.4. 父子容器

- Spring MVC 采用父子容器架构：
  - 父容器（Root WebApplicationContext）：由 ContextLoaderListener 创建，管理 Service、Dao 层 bean
  - 子容器（Servlet WebApplicationContext）：由 DispatcherServlet 创建，管理 Controller、ViewResolver 等 Web 层 bean
- 可见性规则：子容器可以访问父容器中的 bean，父容器不能访问子容器中的 bean
- 实际意义：
  - Controller 可以注入 Service（子访问父），但 Service 不能注入 Controller（父访问子）
  - 多个 DispatcherServlet 可以共享同一个父容器中的 Service 层 bean
- Spring Boot 中的变化：默认只有一个 ApplicationContext，不再区分父子容器

# 10. SPEL

- Spring Expression Language（SpEL）是 Spring 框架核心的表达式引擎
- 编程式使用：
  ```java
  ...
  ```
- 底层机制：SpEL 表达式被编译为 AST（抽象语法树），通过 EvaluationContext 提供变量和函数的解析
- 核心接口：
  - ExpressionParser：解析表达式字符串为 Expression 对象
  - Expression：表达式对象，可以求值
  - EvaluationContext：求值上下文，提供变量、函数、类型转换等
- 在 Spring 内部的应用：@Value、@Cacheable(key="")、@PreAuthorize、XML 配置中的 `#{}`
