...
> **补充：关系型内存数据库 SAP HANA**
> - 也有把关系型数据库和内存数据库结合起来的方案，如SAP HANA
> - 将数据存储在内存中，同时保留关系型数据库的SQL接口和ACID事务特性
> - 支持行存储和列存储混合引擎，适合OLTP和OLAP混合场景
> - 内置数据压缩，实际内存占用远小于原始数据量
> - 缺点：成本极高，搭建集群动辄上亿，中小企业难以承受
...
- CAP
  ...
  - 运用
    ...
    - 所以我们只能 **在一致性和可用性之间进行权衡**
      >多对于大多数web应用，其实并不需要强一致性。因此牺牲C换取P，这是目前分布式数据库产品的方向。
    - **没有NoSQL系统能同时保证这三点**。
  - 实际示例
    - CA:
...
Redis Bitmap 通过字符串类型实现位操作，适用于签到统计、在线状态等需要高效存储大量布尔值的场景。
...
[配置文件(翻译)](./_reference/redis-config.txt)
...
用来判断数据是否过期。Redis 使用一个独立的哈希表（过期字典）保存所有设置了过期时间的 key 及其到期时间戳。
...
- Stream核心特性：
  - 消息持久化：消息存储在Redis中，不会因消费者断开而丢失
  - 消费者组(Consumer Group)：支持多个消费者组独立消费同一Stream
  - 消息确认(ACK)：消费者处理完消息后需确认，未确认的消息可被重新投递
  - 阻塞读取：消费者可阻塞等待新消息到来（类似BLPOP）

- 底层数据结构：
  - Stream内部使用Rax(基数树/压缩前缀树)作为索引结构
  - 每个Rax节点指向一个listpack（紧凑列表，ziplist的替代者）
  - 消息ID格式：`<毫秒时间戳>-<序号>`，如 `1638886400000-0`
  - Rax以消息ID为key进行索引，支持高效的范围查询

- 常用命令：
  - `XADD key ID field value [field value ...]`：追加消息
  - `XREAD COUNT n STREAMS key ID`：读取消息
  - `XGROUP CREATE key groupname ID`：创建消费者组
  - `XREADGROUP GROUP groupname consumer COUNT n STREAMS key >`：组内消费
  - `XACK key groupname ID`：确认消息
...
- 说明：HyperLogLog是一种基数估算算法，用于统计集合中不重复元素的个数
  - 优势：只需要极小的内存（每个key最多占用12KB）即可统计2^64个不同元素
  - 误差率：标准误差为0.81%，对于大多数统计场景完全可接受
  - 不存储元素本身，只存储计算基数所需的状态信息
...
- 与Set的对比：
  - Set存储100万个用户ID约占80MB内存
  - HyperLogLog无论存多少数据始终只占12KB
  - 代价是HyperLogLog无法获取具体元素，且存在误差
...
- 说明：Redis GEO模块基于GeoHash算法实现地理位置相关功能
  - 原理：将二维的经纬度坐标映射为一维的字符串编码
  - 编码方式：交替对经度和纬度进行二分，用0/1表示区间，最终转换为Base32字符串
  - 特性：GeoHash编码越长精度越高，前缀相同的编码地理位置相近

- 常用命令：
  - `GEOADD key longitude latitude member`：添加地理位置
  - `GEODIST key member1 member2 [unit]`：计算两点距离
  - `GEORADIUS key longitude latitude radius unit`：查找指定半径内的成员
  - `GEOSEARCH key FROMMEMBER member BYRADIUS radius unit`：Redis 6.2+新命令
...
- 底层实现：
  - GEO底层使用zset存储，score为GeoHash编码的52位整数
  - 因此可以用ZRANGEBYSCORE进行范围查询
  - 可以存储的地理位置精度约为0.6米
...
> 多个客户端同时对同一个key执行"读取-修改-写回"操作
> 例如：两个客户端同时读取库存为10，各减1后写回9，实际应为8

## 9.1. watch乐观锁

乐观锁，注意不要在分片集群中使用

- Redis事务(MULTI/EXEC) + WATCH乐观锁
  - WATCH监控key，如果key在事务执行前被修改，事务自动失败
  - 适用于竞争不激烈的场景，类似CAS操作
...
Redis Cluster 采用去中心化的对等架构，各节点通过 Gossip 协议通信，自动完成槽分配和故障转移。
...
- Redis Cluster搭建基本步骤：
  - 1.准备至少6个Redis节点（3主3从是最小推荐配置）
  - 2.修改每个节点的配置：`cluster-enabled yes`、`cluster-config-file`、`cluster-node-timeout`
  - 3.启动所有节点
  - 4.使用 `redis-cli --cluster create` 命令创建集群并分配槽位
  - 5.验证集群状态：`redis-cli --cluster check`
...
常见的三种缓存读写策略，用于保证缓存与数据库之间的数据一致性和访问效率。

- 三种缓存策略概述：
  - Cache Aside（旁路缓存）：应用层同时管理缓存和数据库，最常用
  - Read/Write Through（读写穿透）：缓存层代理数据库操作，对应用透明
  - Write Behind（异步写回）：只写缓存，异步批量写数据库，性能最高但风险大

- 选择建议：
  - 读多写少 + 一致性要求高：Cache Aside
  - 希望简化应用逻辑：Read/Write Through
  - 写多读少 + 可容忍数据丢失：Write Behind

<https://javaguide.cn/database/redis/3-commonly-used-cache-read-and-write-strategies.html>

### 12.1.1. 旁路缓存模式

Cache Aside Pattern，应用程序直接与缓存和数据库交互：读时先查缓存，未命中则查数据库并回填缓存；写时先更新数据库再删除缓存。

### 12.1.2. 读写穿透

Read/Write Through Pattern，缓存层作为数据访问的唯一入口，由缓存层自身负责与数据库的同步操作。

### 12.1.3. 异步缓存写入

Write Behind Pattern，只更新缓存，由缓存层异步批量将数据写回数据库，提高写入性能但存在数据丢失风险。
...
- 解决方案：RedLock算法（Redis作者antirez提出）
  - 部署N个（建议5个）完全独立的Redis主节点（非主从，非集群）
  - 客户端按顺序向N个节点请求加锁（使用相同的key和随机value）
  - 如果在大多数节点（>=N/2+1，如5个中的3个）上成功获取锁，且总耗时小于锁的有效期，则认为加锁成功
  - 解锁时向所有节点发送释放锁请求

- RedLock的争议：
  - 分布式系统专家Martin Kleppmann对RedLock提出质疑
  - 主要问题：依赖系统时钟同步，GC暂停可能导致锁失效
  - antirez进行了回应，认为在合理的时钟偏差下RedLock是安全的

- 其他解决方案：
  - 使用ZooKeeper实现分布式锁（CP模型，更可靠但性能稍低）
  - 使用etcd实现分布式锁（基于Raft共识算法）
  - 业务层面做幂等设计，即使锁失效也不会造成数据错误
...
可利用 HyperLogLog 实现海量用户的 UV（Unique Visitor）去重统计，占用内存极小。

## 12.6. 排行榜

利用 Sorted Set 的分数排序能力，可高效实现各类实时排行榜功能。

## 12.7. 关注列表和粉丝列表

利用 Set 的交集、并集操作，可以实现共同关注、好友推荐等社交关系功能。

## 12.8. 广告弹窗触达频率的控制

利用 Redis 的计数和过期功能，可以精确控制广告对同一用户的展示频次。

## 12.9. 延时队列

利用 Sorted Set 以时间戳为 score 存储任务，通过定时轮询取出到期任务实现延时队列。
...
容器型数据结构（list、set、hash、zset）遵循两条通用规则：不存在时自动创建，无元素时自动删除释放内存。
...
- 双写一致问题核心：
  - 更新数据时如何保证缓存和数据库数据一致
  - 常见方案有四种：先更新DB再删缓存、先删缓存再更新DB、先更新DB再更新缓存、先更新缓存再更新DB
  - 推荐方案：先更新数据库，再删除缓存（Cache Aside Pattern）
  - 终极保障：设置合理的缓存过期时间，保证最终一致性
...
- 文件事件处理器的组成：
  - 多个socket：客户端连接产生的socket
  - IO多路复用程序：监听多个socket，将产生事件的socket放入队列
  - 文件事件分派器：从队列中取出socket，根据事件类型分派给对应的处理器
  - 事件处理器：连接应答处理器、命令请求处理器、命令回复处理器

- 工作流程：
  - 1.客户端连接到Redis，产生AE_READABLE事件，由连接应答处理器处理
  - 2.客户端发送命令，socket产生AE_READABLE事件，由命令请求处理器读取并执行
  - 3.执行完毕需要返回结果时，将socket的AE_WRITABLE事件关联命令回复处理器
  - 4.命令回复处理器将结果写回客户端后，解除AE_WRITABLE事件关联
...
- Redis 6.0多线程工作流程：
  - 主线程负责接收建立连接请求，读事件到来时将socket放入等待队列
  - IO线程并行读取socket数据并解析命令（多线程读）
  - 主线程单线程执行命令（保证线程安全）
  - IO线程并行将执行结果写回socket（多线程写）
  - 核心要点：命令执行仍是单线程，多线程仅用于网络IO读写

- 性能提升效果：
  - 官方测试显示，开启多线程后性能可提升一倍左右
  - 在高并发场景下，网络IO成为瓶颈时效果明显
  - 对于计算密集型命令（如SORT、SUNION）提升不大
