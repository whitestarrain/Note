...
- Nginx的主要应用场景：
  - **反向代理**：作为后端服务的统一入口，隐藏真实服务器地址，支持HTTP/HTTPS
  - **负载均衡**：将请求分发到多台后端服务器，支持轮询/加权/ip_hash/least_conn等策略
  - **静态资源服务**：高效托管HTML/CSS/JS/图片等静态文件，性能远超应用服务器
  - **API网关**：统一管理API路由、限流、认证、跨域等功能
  - **SSL终端**：在Nginx层完成HTTPS加解密，后端服务使用HTTP通信，减轻应用负担
  - **文件服务器**：配合autoindex实现目录浏览和文件下载
  - **动静分离**：静态资源由Nginx直接返回，动态请求转发给后端应用服务器
  - **跨域解决**：通过add_header添加CORS响应头，或通过反向代理避免跨域
  - **灰度发布**：根据请求特征（IP、Cookie、Header）将部分流量路由到新版本
  - **限流防刷**：通过limit_req和limit_conn模块实现请求速率和连接数限制
  - **缓存加速**：proxy_cache将后端响应缓存到本地，减少后端压力
  - **WebSocket代理**：通过升级HTTP连接支持WebSocket长连接转发
