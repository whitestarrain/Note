...
计算机视觉是使计算机能够从图像或视频中获取高层次理解的学科，目标是模拟人类视觉系统的能力。
...
- 膨胀：是图像中的高亮部分进行膨胀，类似于邻域扩张。
- 腐蚀：是原图的高亮部分被腐蚀，类似于邻域被蚕食。
  > ![shijue-11](./image/shijue-11.png)
- 开运算：先腐蚀再膨胀，可以去掉目标外的孤立点。
- 闭运算：先膨胀再腐蚀，可以去掉目标内的孔。

> 通常，当有噪声的图像用阈值二值化后，所得到的边界是很不平滑的，物体区域具有一些错判的孔洞，背景区域散布着一些小的噪声物体，连续的开和闭运算可以显著地改善这种情况。
...
```
阈值法的基本思想是基于图像的灰度特征来计
算一个或多个灰度阈值，并将图像中每个像素
的灰度值与阈值相比较，最后将像素根据比较
结果分到合适的类别中。
```
...
dlib库提供了基于HOG和CNN的人脸检测器，以及68点人脸关键点检测模型。
...
DeepSort是一种多目标跟踪算法，在SORT的基础上加入深度特征匹配，通过外观特征和运动信息实现更稳定的目标关联。
